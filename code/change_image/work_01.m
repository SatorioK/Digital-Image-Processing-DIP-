...
template_I = imread('..\..\data\阻挡666.jpg');

%% 图片大小变化与风格变化
figure, imshow(template_I), title('请选择要替换区域的四个点');
[M, N, ~] = size(template_I);
source_I = make_jiepai(imresize(source_I, [M, N]));
...
%% 利用蒙版将图片放进原图中
mask = im2double(im2gray(imread('..\..\data\mask阻挡666.jpg')));
...
imwrite(uint8(converted_I), '阻挡替换后结果.jpg');
