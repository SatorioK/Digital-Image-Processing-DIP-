...
template_I = imread('..\..\data\书本530.jpg');

%% 图片大小变化与风格变化
figure, imshow(template_I), title('请选择要替换区域的八个点');
[M, N, ~] = size(template_I);
source_I = make_old(imresize(source_I, [M, N]));

%% 空间变换
select_points = ginput(8);
origin_points = [1 1; N 1; 1 M; N M; N/4 1; N*3/4 1; N/4 M; N*3/4 M];
converted_I = morph_tps_wrapper(source_I, template_I, origin_points, select_points);
close;

%% 利用蒙版将图片放进原图中
mask = double(im2gray(imread('..\..\data\mask书本530.jpg'))) / 255;
...
imwrite(uint8(converted_I), '书本替换后结果.jpg');
